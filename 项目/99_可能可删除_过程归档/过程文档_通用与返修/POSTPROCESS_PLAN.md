...
Important update from the final delivery incident:

- Correct dimensions and alpha presence are not enough.
- Exact `#FF00FF` checks are not enough.
- QA must include broad pink/purple residue detection and manual contact-sheet review on white, dark, checkerboard, and in-context backgrounds.
- Fixed-slot transparent PNGs should not be auto-cropped during programmer integration.
